...
#### Scenario: Missing columns raises ValueError

- GIVEN a DataFrame missing one or more required feature columns
- WHEN `predict(model, df)` is called
- THEN a `ValueError` SHALL be raised
- AND the error message SHALL name the missing columns

#### Scenario: Empty DataFrame returns empty result

- GIVEN an empty DataFrame with correct columns
- WHEN `predict(model, df)` is called
- THEN an empty DataFrame SHALL be returned with the expected output schema
...
- THEN the 4 corresponding `affix_{id}` flags SHALL be 1.0
- AND all other affix flags SHALL be 0.0
...
#### Scenario: NULL KPI columns are imputed

- GIVEN a run with `deficit_ratio = NULL` and `interrupts_per_minute = NULL`
- WHEN feature engineering runs
- THEN those columns SHALL be imputed with 0.0
